...
class PNGOverlay:
    ...
    def _addTransparentImage(
        self, src
    ):  # 回転時にクロップしないように、予め画像の透明色領域を周囲に加える
    ...
    def _preProcessingImage(
        self, src_bgra
    ):  # BGRA画像をBGR画像(src)とA画像(mask)に分け、オーバーレイ時に必要な情報を保持
    ...
    def _transformImage(
        self,
    ):  # 各メソッドでバラバラに行わず、一括でリサイズと回転をシリーズに行う必要がある
        # ---------------------------------
        # resize
        # ---------------------------------
        self.src_bgra = cv2.resize(
            self.src_init, dsize=None, fx=self.size_value, fy=self.size_value
        )  # 倍率で指定
    ...
    def show(self, dst, x, y):  # dst画像にsrcを重ね合わせ表示。中心座標指定
        ...
        if self.flag_transformImage:
            self._transformImage()

        # 前処理が必要な場合は実行
        if self.flag_preProcessingImage:
        ...
        x2, y2 = (
            x1 + self.width,
            y1 + self.height,
        )  # widthやheightを加える計算式にしないと１ずれてエラーになる場合があるので注意
        a1, b1 = 0, 0
        ...
        # dstのフレームからのはみ出しを補正
        x1, y1, x2, y2, a1, b1, a2, b2 = self._correctionOutofImage(
            dst, x1, y1, x2, y2, a1, b1, a2, b2
        )

        # Aの割合だけ src 画像を dst にブレンド
        dst[y1:y2, x1:x2] = self.src[b1:b2, a1:a2] * self.mask[b1:b2, a1:a2] + dst[
            y1:y2, x1:x2
        ] * (1 - self.mask[b1:b2, a1:a2])

    def _correctionOutofImage(
        self, dst, x1, y1, x2, y2, a1, b1, a2, b2
    ):  # x, y座標がフレーム外にある場合、x, y及びa, bを補正する
        dst_height, dst_width, _ = dst.shape
        if x1 < 0:
            a1 = -x1
            x1 = 0
